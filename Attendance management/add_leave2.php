...
if(($_SESSION['ROLE']==1) && ($_SESSION['DEPARTMENT_ID'])){ 
	$sql="select `hod_leave`.*, employee.name,employee.role=2  as eid  from `hod_leave`,employee where `hod_leave`.employee_id=employee.id order by `hod_leave`.id desc";
}else{
	$eid=$_SESSION['USER_ID'];
	$sql="select `hod_leave`.*, employee.name ,employee.id as eid from `hod_leave`,employee where `hod_leave`.employee_id='$eid' and `hod_leave`.employee_id=employee.id order by `hod_leave`.id desc";
}
                                 ...
                                 <thead>
                                    <tr>
									   ...
									   <th width="15%">Description</th>
									    ...
									    <th width="10%">Alternate period</th>
